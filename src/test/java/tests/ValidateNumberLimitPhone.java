...
public class ValidateNumberLimitPhone extends BaseTest {
    ...
    public void validateContactName(){
        try {
            ...
            contactPage.sendKeysToContactName("number limit");
        }catch (NoSuchElementException exception){
    ...
    // Burada sıra sıra 100 digite ulaşınca duracak şekilde tasarladığım algoritmayı key olarak gönderirim
    // Çünkü number limit test case sınıfıdır bu test sınıfı
    @Test(priority = 6)
    ...
    public void validateContactEmail(){
        try {
            ...
            contactPage.sendKeysToContactEmail("test4@mail.com");
        }catch (NoSuchElementException exception){
    ...
    public void validateChecksNewContact(){
        try{
            ...
            Assert.assertEquals(homePage.dynamicCheckSavedName("number limit").getText(),"number limit");
        }catch (NoSuchElementException exception){
